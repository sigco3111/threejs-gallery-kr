...
import { float as float2, sin as sin2, smoothstep, uniform, vec2 as vec22 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
...
function createSandMaterial(applyCaustics) {
  ...
  const base = mix2(vec32(0.48, 0.43, 0.33), vec32(0.58, 0.54, 0.43), tone);
  material.colorNode = mix2(base, vec32(0.33, 0.4, 0.3), patchTone.smoothstep(0.62, 0.85).mul(0.5));
  material.normalNode = Fn2(() => {
    const slope = seabedRippleSlope(xz).mul(float3(1).sub(seabedRippleBakeFlat));
    const localNormal = normalize(normalGeometry.add(vec32(slope.x, 0, slope.y)));
