...
import { Fn as Fn2, cos, sin, vec3 as vec32 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
  ...
  constructor(ping, pong, n) {
    ...
    const makeAxis = (source, dest, horizontal) => {
      ...
      return Fn3(() => {
        ...
        const output2 = horizontal ? ivec2(int(lane), line) : ivec2(line, int(lane));
        textureStore(dest, output2, shared.element(lane));
      })().compute(n * n, [n]);
    };
...
import { MeshStandardNodeMaterial } from "https://esm.sh/three@0.185.1/webgpu?deps=three@0.185.1";
import {
  cameraPosition,
...
import { TessellateModifier } from "https://esm.sh/three@0.185.1/examples/jsm/modifiers/TessellateModifier.js?deps=three@0.185.1";
...
function clipGeometryAboveY(source, minimumY) {
  ...
  const output2 = new Map(attributes.map(([name]) => [name, []]));
  ...
  const emit = (vertex) => {
    for (const [name] of attributes) output2.get(name)?.push(...vertex[name]);
  };
  ...
  for (const [name, attribute] of attributes) {
    result.setAttribute(
      name,
      new Float32BufferAttribute(output2.get(name) ?? [], attribute.itemSize)
    );
  }
...
import { bloom } from "https://esm.sh/three@0.185.1/examples/jsm/tsl/display/BloomNode.js?deps=three@0.185.1";
...
import { Fn as Fn6, Loop as Loop2, float as float6, fract as fract2, dot as dot2, floor, mix as mix4, sin as sin3, vec2 as vec25, vec3 as vec35 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
...
import { dot as dot4, float as float9, max as max5, mix as mix7, normalize as normalize4, pow as pow3, smoothstep as smoothstep6, vec2 as vec26, vec3 as vec37 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";

// docs/skills/threejs-spectral-ocean/examples/submerged-snell-ocean/source/sky-radiance.ts
import { Fn as Fn7, dot as dot3, float as float7, max as max4, mix as mix5, normalize as normalize3, pow as pow2, smoothstep as smoothstep5, vec3 as vec36 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
...
import { MeshBasicNodeMaterial as MeshBasicNodeMaterial3 } from "https://esm.sh/three@0.185.1/webgpu?deps=three@0.185.1";
import {
  Fn as Fn9,
  If as If3,
...
import { float as float10, sin as sin4, smoothstep as smoothstep7, uniform as uniform7, vec2 as vec27 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
...
import { BufferAttribute, BufferGeometry as BufferGeometry2 } from "https://esm.sh/three@0.185.1?external";
...
function createOceanSurfaceMaterial(sim, timeUniform, options) {
  ...
  material.mrtNode = mrt2({ normal: vec47(normalView2, 0) });
...
import { DataTexture as DataTexture2, FloatType as FloatType2, NearestFilter as NearestFilter3, RGBAFormat as RGBAFormat3 } from "https://esm.sh/three@0.185.1?external";
...
import { uniform as uniform9 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
...
import { StorageTexture as StorageTexture3 } from "https://esm.sh/three@0.185.1/webgpu?deps=three@0.185.1";
import {
  Fn as Fn10,
  float as float12,
...
import { MeshStandardNodeMaterial as MeshStandardNodeMaterial3 } from "https://esm.sh/three@0.185.1/webgpu?deps=three@0.185.1";
import {
  Fn as Fn11,
  float as float13,
  mix as mix9,
...
import { MeshBasicNodeMaterial as MeshBasicNodeMaterial4, PMREMGenerator } from "https://esm.sh/three@0.185.1/webgpu?deps=three@0.185.1";
import { float as float14, normalize as normalize7, positionLocal as positionLocal3 } from "https://esm.sh/three@0.185.1/tsl?deps=three@0.185.1";
