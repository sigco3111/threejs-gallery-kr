...
const SDF_SIZE = 512
// must match SDF_EXTENT / BASE_SHORE_X in tsl.js
...
function bakeSDF(coast) {
  ...
  for (let iz = 0; iz < SDF_SIZE; iz++) {
    ...
    for (let ix = 0; ix < SDF_SIZE; ix++) {
      ...
      const o = (iz * SDF_SIZE + ix) * 4
      data[o] = sd
      data[o + 3] = 1
    }
  }
  // Keep the r16float SDF contract. Half-float stays filterable on WebGPU
  // without depending on the optional float32-filterable feature.
  const half = new Uint16Array(data.length)
  for (let i = 0; i < data.length; i++) half[i] = THREE.DataUtils.toHalfFloat(data[i])
  const texture = new THREE.DataTexture(half, SDF_SIZE, SDF_SIZE, THREE.RGBAFormat, THREE.HalfFloatType)
  texture.wrapS = texture.wrapT = THREE.ClampToEdgeWrapping
  texture.magFilter = THREE.LinearFilter
  texture.minFilter = THREE.LinearFilter
  texture.generateMipmaps = false
  texture.needsUpdate = true
  coast.sdfTexture = texture
}

function uploadMainTable(coast) {
  const { main } = coast
  ...
  const texture = new THREE.DataTexture(data, MAIN_TABLE_N, 1, THREE.RGBAFormat, THREE.FloatType)
  texture.magFilter = THREE.NearestFilter
  texture.minFilter = THREE.NearestFilter
  texture.generateMipmaps = false
  texture.needsUpdate = true
  coast.mainTableTexture = texture
}
