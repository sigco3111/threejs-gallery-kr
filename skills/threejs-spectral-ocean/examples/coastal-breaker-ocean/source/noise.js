...
export function generateGravityNoiseSet(opts = {}) {
  ...
  for (const v of variants) {
    ...
    textures.push(createNoiseTexture(size, v.h, v.d, hx, hy))
    heights.push(v.h)
  }
...
export function generateCapillaryNoiseTexture(opts = {}) {
  const size = opts.size ?? 256
  ...
  return {
    texture: createNoiseTexture(size, h, null, hx, hy),
    size,
...
export function generateFoamPatternTexture(opts = {}) {
  const size = opts.size ?? 256
  ...
  for (let i = 0; i < n; i++) {
    rgba[i * 4] = Math.round(density[i] * 255)
    rgba[i * 4 + 3] = 255
  }
  const texture = new THREE.DataTexture(rgba, size, size, THREE.RGBAFormat, THREE.UnsignedByteType)
...
function createNoiseTexture(size, r, g, b, a) {
  const n = size * size
  const data = new Uint16Array(n * 4)
  for (let i = 0; i < n; i++) {
    data[i * 4] = THREE.DataUtils.toHalfFloat(r[i])
    data[i * 4 + 1] = THREE.DataUtils.toHalfFloat(g ? g[i] : 0)
    data[i * 4 + 2] = THREE.DataUtils.toHalfFloat(b[i])
    data[i * 4 + 3] = THREE.DataUtils.toHalfFloat(a[i])
  }
